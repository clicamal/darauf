<?php

declare(strict_types=1);

use Clicamal\Darauf\Models\DidDocument;
use Clicamal\Darauf\Models\VerificationMethod;

beforeEach(function () {
    $this->artisan('migrate', [
        '--path' => realpath(__DIR__.'/../../database/migrations'),
        '--realpath' => true,
    ])->assertSuccessful();
});

function ed25519User(string $suffix): array
{
    $did = 'did:darauf:'.$suffix;

    $document = DidDocument::factory()->create([
        'did_document_id' => $did,
    ]);

    $keyPair = ed25519KeyPair();

    VerificationMethod::factory()->create([
        'verification_method_id' => $did.'#key-1',
        'did_document_id' => $document->id,
        'serialized' => json_encode([
            'id' => $did.'#key-1',
            'type' => 'Ed25519VerificationKey2020',
            'controller' => $did,
            'publicKeyMultibase' => $keyPair['publicKeyMultibase'],
        ]),
    ]);

    return [
        'did' => $did,
        'private' => $keyPair['private'],
        'publicKeyMultibase' => $keyPair['publicKeyMultibase'],
    ];
}

it('returns a challenge for a registered document', function () {
    $user = ed25519User('alice');

    $response = $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [
        'didDocumentId' => $user['did'],
    ]);

    $response->assertCreated();

    $challenge = $response->json();

    expect($challenge)->toHaveKeys(['id', 'string'])
        ->and(Cache::has("darauf_ed25519_challenge:{$challenge['id']}"))->toBeTrue();
});

it('rejects a challenge for an unregistered document', function () {
    $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [
        'didDocumentId' => 'did:darauf:ghost',
    ])
        ->assertUnprocessable()
        ->assertJsonPath('message', __('darauf::messages.error.did_document_not_found'));
});

it('rejects a did document id without an ed25519 verification method', function () {
    ed25519User('alice');

    $document = DidDocument::factory()->create([
        'did_document_id' => 'did:darauf:noed25519',
    ]);

    VerificationMethod::factory()->create([
        'verification_method_id' => 'did:darauf:noed25519#key-1',
        'did_document_id' => $document->id,
        'serialized' => json_encode([
            'id' => 'did:darauf:noed25519#key-1',
            'type' => 'RSA',
            'controller' => 'did:darauf:noed25519',
            'publicKeyMultibase' => 'z'.base64url_encode('some-key'),
        ]),
    ]);

    $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [
        'didDocumentId' => 'did:darauf:noed25519',
    ])
        ->assertUnprocessable()
        ->assertJsonPath('message', __('darauf::challenge_managers.ed25519.verification_method_not_found'));
});

it('rejects a missing didDocumentId in the generate endpoint', function () {
    $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [])
        ->assertUnprocessable()
        ->assertJsonValidationErrors(['didDocumentId']);
});

it('rejects an unsupported verification method', function () {
    $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'secp256k1']), [
        'didDocumentId' => 'did:darauf:alice',
    ])
        ->assertUnprocessable()
        ->assertJsonPath('message', __('darauf::messages.error.verification_method_not_supported'));
});

it('registers the named challenge generate route', function () {
    expect(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']))
        ->toBe('http://localhost/api/darauf/v0.1.2/challenge/generate/Ed25519VerificationKey2020');
});

it('accepts a valid signature in the verify endpoint', function () {
    $user = ed25519User('alice');

    $challengeResponse = $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [
        'didDocumentId' => $user['did'],
    ])->assertCreated();

    $challenge = $challengeResponse->json();

    $signature = sodium_crypto_sign_detached($challenge['string'], $user['private']);

    $this->postJson(route('darauf.verification.challenge.verify', ['method' => 'Ed25519VerificationKey2020']), [
        'challengeId' => $challenge['id'],
        'signature' => base64_encode($signature),
    ])->assertOk()
        ->assertJsonPath('message', __('darauf::messages.success.did_subject_authenticated'));
});

it('rejects an invalid signature in the verify endpoint', function () {
    $user = ed25519User('alice');

    $challengeResponse = $this->postJson(route('darauf.verification.challenge.generate', ['method' => 'Ed25519VerificationKey2020']), [
        'didDocumentId' => $user['did'],
    ])->assertCreated();

    $challenge = $challengeResponse->json();

    $signature = sodium_crypto_sign_detached('not-the-challenge', $user['private']);

    $this->postJson(route('darauf.verification.challenge.verify', ['method' => 'Ed25519VerificationKey2020']), [
        'challengeId' => $challenge['id'],
        'signature' => base64_encode($signature),
    ])
        ->assertUnauthorized()
        ->assertJsonPath('message', __('darauf::messages.error.verification_failed'));
});

it('rejects a challenge that was never generated', function () {
    $this->postJson(route('darauf.verification.challenge.verify', ['method' => 'Ed25519VerificationKey2020']), [
        'challengeId' => 'missing-challenge-id',
        'signature' => base64_encode('signature'),
    ])
        ->assertUnauthorized()
        ->assertJsonPath('message', __('darauf::messages.error.challenge_not_found'));
});

it('rejects a missing payload in the verify endpoint', function () {
    $this->postJson(route('darauf.verification.challenge.verify', ['method' => 'Ed25519VerificationKey2020']), [])
        ->assertUnprocessable()
        ->assertJsonValidationErrors(['challengeId', 'signature']);
});

it('registers the named verify route', function () {
    expect(route('darauf.verification.challenge.verify', ['method' => 'Ed25519VerificationKey2020']))
        ->toBe('http://localhost/api/darauf/v0.1.2/challenge/verify/Ed25519VerificationKey2020');
});
