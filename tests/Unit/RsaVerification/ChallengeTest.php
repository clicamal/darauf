...
it('generates a challenge with an id and a string', function () {
    $challenge = RsaChallenge::generate('public-key');
...
it('verifies a valid signature', function () {
    ...
    $challenge = RsaChallenge::generate($publicKey);
...
it('rejects an invalid signature', function () {
    ...
    expect(RsaChallenge::verify($challenge['challengeId'], $signature))->toBe(0);
});

it('throws when the challenge is not found', function () {
    RsaChallenge::verify('missing-challenge-id', 'signature');
...
it('is single use', function () {
    ...
    expect(fn () => RsaChallenge::verify($challenge['challengeId'], $signature))
